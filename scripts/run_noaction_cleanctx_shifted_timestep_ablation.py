...
BENCHMARK_DIR = ROOT / "data" / "benchmarks" / "noaction_cleanctx_shifted_timestep_ablation_seed231_all5"
LOG_DIR = ROOT / "data" / "modal_logs" / "noaction_cleanctx_shifted_timestep_ablation_seed231_all5"

UNIFORM_RUN_NAME = "ltx2b_distilled098_waymo24_noaction_visual_lora_r16_seed231_full7992_lr5e6_2epochs_steps15984"
SHIFTED_RUN_NAME = "ltx2b_dist098_waymo24_noaction_shifted_lognormal_r16_seed231_lr5e6_steps1000"
...
CONDITIONS = [
    ...
    Condition(
        ...
        lora_run_name=SHIFTED_RUN_NAME,
        checkpoints=SHIFTED_CHECKPOINTS,
    ),
]
...
def train_shifted_ablation(seed: int, max_steps: int) -> None:
    log_path = LOG_DIR / "training" / "shifted_lognormal_steps1000.log"
...
def generate_all(max_workers: int, seed: int) -> None:
    jobs = [(condition, step) for condition in CONDITIONS for step in condition.checkpoints]
...
def build_manifest(seed: int) -> Path:
    ...
    manifest = {
        ...
        "description": (
            "Clean-context no-action ablation: regenerate existing uniform checkpoints with "
            "image_cond_noise_scale=0.0 and compare to a shifted-lognormal timestep short run."
        ),
        "seed": seed,
        "context_frames": 49,
        "future_frames": 72,
    ...
    manifest_path = BENCHMARK_DIR / "manifest_noaction_cleanctx_shifted_timestep_seed231_all5.json"
...
def parse_model_mode(model_mode: str) -> tuple[str, int]:
    for condition in CONDITIONS:
        prefix = condition.key + "_"
        if model_mode.startswith(prefix):
            suffix = model_mode[len(prefix) :]
            return condition.key, step_to_int("base" if suffix == "base" else suffix.replace("step", "step_"))
...
def merge_summaries() -> list[dict[str, Any]]:
    ...
    label_by_key = {condition.key: condition.label for condition in CONDITIONS}
    ...
    write_csv(BENCHMARK_DIR / "noaction_cleanctx_shifted_timestep_summary_with_fvd.csv", merged)
    return merged
...
def plot_clear_graphs(summary_csv: Path) -> None:
    cmd = [
        ...
        "--title",
        "No-Action Clean-Context: Uniform vs Shifted Log-Normal Timesteps",
    ]
...
def write_report(rows: list[dict[str, Any]]) -> Path:
    ...
    for step in [500, 1000]:
        ...
        comparisons[f"step_{step:06d}"] = {
            "future_psnr_shifted_minus_uniform": as_float(shifted, "mean_future_psnr") - as_float(uniform, "mean_future_psnr"),
            "future_ssim_shifted_minus_uniform": as_float(shifted, "mean_future_global_ssim")
            - as_float(uniform, "mean_future_global_ssim"),
            "sharpness_ratio_shifted_minus_uniform": as_float(shifted, "mean_sharpness_ratio_generated_over_reference")
    ...
    report = {
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "summary_csv": str(BENCHMARK_DIR / "noaction_cleanctx_shifted_timestep_summary_with_fvd.csv"),
        "comparisons": comparisons,
    }
    path = BENCHMARK_DIR / "noaction_cleanctx_shifted_timestep_report.json"
...
def main() -> None:
    ...
    if not args.skip_training:
        train_shifted_ablation(seed=args.seed, max_steps=args.max_steps)
    ...
    summary_csv = BENCHMARK_DIR / "noaction_cleanctx_shifted_timestep_summary_with_fvd.csv"
